...
std::string getErrorText() {
  ...
  auto errorText = std::string(
      std::string_view(buf_mem->data, buf_mem->data + buf_mem->length));
  BIO_free(bio);
  return errorText;
}
...
std::string getSignatureHEX64(const char *key, const uint_fast8_t keySize,
                              const std::string_view data) {
  std::array<unsigned char, SHA256_DIGEST_LENGTH> hmacResult;
  unsigned hmacLength;
  HMAC(EVP_sha256(), key, keySize,
       const_cast<unsigned char *>(
           reinterpret_cast<const unsigned char *>(data.data())),
       std::size(data), hmacResult.data(), &hmacLength);
  ...
  const std::string signature{
      std::string_view(hexstring.data(), SHA256_DIGEST_LENGTH * 2)};
  return signature;
}

std::string getSignatureBase64(const char *key, const uint_fast8_t keySize,
                               const std::string_view data, bool decodeKey) {
  std::array<unsigned char, SHA256_DIGEST_LENGTH> hmacResult;
  unsigned hmacLength;
  if (!decodeKey) {
    HMAC(EVP_sha256(), key, keySize,
         reinterpret_cast<const unsigned char *>(data.data()), std::size(data),
         hmacResult.data(), &hmacLength);
  } else {
    auto decodedKey = base64Decode(std::string_view{key, keySize});
    HMAC(EVP_sha256(), decodedKey.data(), decodedKey.size(),
         reinterpret_cast<const unsigned char *>(data.data()), std::size(data),
         hmacResult.data(), &hmacLength);
  }
  return base64Encode(std::string_view{
      reinterpret_cast<char *>(hmacResult.data()), hmacLength});
}
...
uint32_t generateMaskingKey() { return std::rand(); };
...
std::string getHEX64Signature(const std::string &secretKey,
  ...
  HMAC(EVP_sha256(), secretKey.data(), secretKey.length(),
       reinterpret_cast<const unsigned char *>(payload.data()),
       payload.length(), hmacResult, &hmacLength);
...
std::string generateWebSocketAcceptKey(const std::string &secKey) {
  ...
  SHA1(reinterpret_cast<const unsigned char *>(combinedKey.data()),
       combinedKey.size(), hash);
  return base64Encode(
      std::string_view(reinterpret_cast<char *>(hash), SHA_DIGEST_LENGTH));
}

std::string getES256SignatureBase64(std::string_view privateKeyPem,
                                    std::string_view payload) {
  auto signer = Signer::create(privateKeyPem);
  return signer->sign(payload);
}

namespace {
std::vector<unsigned char>
convertEcdsaDerToRawRs(const unsigned char *derSignature,
                       size_t derSignatureLen, size_t partLength) {
  const unsigned char *parsePtr = derSignature;
  ECDSA_SIG *ecdsaSig = d2i_ECDSA_SIG(nullptr, &parsePtr, derSignatureLen);
  if (!ecdsaSig) {
    throw std::runtime_error("Failed to parse DER ECDSA signature.");
  }
  ...
  std::vector<unsigned char> rawSignature(partLength * 2);
  const int rWritten =
      BN_bn2binpad(r, rawSignature.data(), static_cast<int>(partLength));
  const int sWritten = BN_bn2binpad(s, rawSignature.data() + partLength,
                                    static_cast<int>(partLength));
  ECDSA_SIG_free(ecdsaSig);

  if (rWritten != static_cast<int>(partLength) ||
      sWritten != static_cast<int>(partLength)) {
    throw std::runtime_error(
        "Failed to convert ECDSA signature to raw RS format.");
  }
...
struct SHA256Signer : public Signer {
  SHA256Signer(std::string_view key) : key(key) {
    ...
    if (nullptr == HMAC(EVP_sha256(), key.data(), key.size(),
                        reinterpret_cast<const unsigned char *>(key.data()),
                        key.size(), test, nullptr)) {
      throw std::runtime_error("Invalid key");
    };
  }

  bool isSessionSigner() const override { return false; }
...
struct SSLSigner : public Signer {
  ...
  BIO *bio = nullptr;
  bool isEs256 = false;
  size_t es256PartLength = 0;
  bool isSessionSigner() const override { return true; }
  ...
  SSLSigner(std::string_view pem_key) {
    ...
    bio = BIO_new_mem_buf(pem_key.data(), pem_key.size());
    if (!bio) {
    ...
    if (EVP_PKEY_base_id(pkey) == EVP_PKEY_EC) {
      ...
      const int groupNameLoaded = EVP_PKEY_get_utf8_string_param(
          pkey, OSSL_PKEY_PARAM_GROUP_NAME, groupName.data(), groupName.size(),
          &groupNameLen);

      if (groupNameLoaded == 1) {
        std::string_view group{groupName.data(), groupNameLen};
      ...
      if (!isEs256 && EVP_PKEY_bits(pkey) == 256) {
        isEs256 = true;
        es256PartLength = 32;
      }
    }
...
private:
  std::string sign(std::string_view data) override {
    ...
    const EVP_MD *digest = isEs256 ? EVP_sha256() : nullptr;
    if (EVP_DigestSignInit(mdctx, NULL, digest, NULL, pkey) != 1) {
    ...
    if (isEs256) {
      signature = convertEcdsaDerToRawRs(signature.data(), signature.size(),
                                         es256PartLength);
    }
